...
// Set environment for anchor program
process.env.ANCHOR_PROVIDER_URL = "https://api.devnet.solana.com"
process.env.ANCHOR_WALLET = path.join(__dirname, './wallets/relayer.json')

const KEYPAIR_PATH = path.join(__dirname, 'keypairs');
const idl = JSON.parse(fs.readFileSync(path.join(__dirname, './settlement_app.json'), 'utf8'));
...
async function main() {
  ...
  async function postMessage(taskId: anchor.BN, sender: string, message: string) {
  ...
  async function getMessage(taskId: anchor.BN) {
    ...
    const messageString = Buffer.from(retrievedMessage, 'utf-8').toString();
